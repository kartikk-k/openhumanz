...
const { createComposioClient } = require('./src/main/modules/composio/client.ts');
...
async function main() {
  ...
  if (!isConnected) {
    ...
    try {
      ...
      console.log(
        c.red(`  did not connect: ${error instanceof Error ? error.message : String(error)}`),
      );
      process.exit(1);
    }
  }
  ...
  if (tools.length === 0) {
    console.log(c.red('  none returned — check the toolkit slug / connection.'));
  } else {
